...
RSpec.describe CrazyEightsGame, type: :model do
  ...
  it_behaves_like "a platform game",
    ...
    legal_turn: ->(game) do
      state = game.state
      playable = CardGame::Card.new("3", state.discard.active_card.suit)
    ...
    winning_turn: ->(game, winner) do
      ...
      winning_card = CardGame::Card.new("A", "Spades")
      champion.cards = [ winning_card ]
      state.discard.active_card = CardGame::Card.new("2", "Spades")
