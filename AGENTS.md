...
- **CSS is served by Propshaft, not bundled.** Styling uses the **Optics** design system (via CDN) + **design tokens** + **BEM** class naming, one block per file in `app/assets/stylesheets/components/`. **Token tiers: `--op-*` (Optics, non-color only) → `--gp-*` (project; *all* color) → `--_gp-*` (private to one component).** Color is never taken from Optics — it is defined in `core/theme.css` as `--gp-color-*`, and a bridge block there feeds those values back to the `--op-color-*` tokens Optics reads internally so its own components are painted from our palette. Referencing `--op-color-*` anywhere outside that bridge is a bug (`grep -rn -- '--op-color-' app/assets/stylesheets/ | grep -v core/theme.css` should be empty). An older `--gf-*` prefix was retired; if you see it, it's stale. To add styles you just drop a `.css` file there — `stylesheet_link_tag :app` auto-includes it; there is no manifest, `@import`, or CSS build step. The empty `application.css` and webpack CSS build are unused — don't "fix" them. See [docs/frontend.md](docs/frontend.md).
