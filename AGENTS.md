...
- **`:chrome`-tagged system specs can't fake "offline" for anything the service worker fetches.** Selenium's `network_conditions=` only emulates offline on the page's own DevTools target — `app/views/pwa/service-worker.js` runs on a separate CDP target, so its `fetch()` still reaches the real server and the "show cached offline page" path never triggers. Test SW caching directly (`caches.match(...)`) instead of simulating a real failed navigation. See `docs/handoffs/2026-07-22-offline-spec-flake.md`.
