...
- **CSS is served by Propshaft, not bundled.** Styling uses the **Optics** design system (vendored locally in `vendor/optics/`, being phased out — see `docs/plans/own-the-tokens.md`) + **design tokens** + **BEM** class naming, one block per file in `app/assets/stylesheets/components/`. **Token tiers: `--gp-*` (project; all color *and* non-color primitives) → `--_gp-*` (private to one component); `--op-*` is now only Optics' own internal tokens**, read by its own not-yet-replaced components (button, sidebar, form, dialog). Color is never taken from Optics — it is defined in `core/theme.css` as `--gp-color-*` (a generated, dark-only OKLCH ramp), and a bridge block there feeds those values back to the `--op-color-*` tokens Optics reads internally so its own components are painted from our palette. Referencing `--op-color-*` anywhere outside that bridge is a bug (`grep -rn -- '--op-color-' app/assets/stylesheets/ | grep -v core/theme.css` should be empty). Non-color primitives (space/font/radius/border-width/shadow) live in `core/tokens/scale.css` as `--gp-*` too. An older `--gf-*` prefix was retired; if you see it, it's stale. To add styles you just drop a `.css` file there — `stylesheet_link_tag :app` auto-includes it; there is no manifest, `@import`, or CSS build step. The empty `application.css` and webpack CSS build are unused — don't "fix" them. See [docs/frontend.md](docs/frontend.md).
- **Never use `color-mix()` to derive a color.** If the palette doesn't have the shade you need, either tweak the neutral scale's own numbers (`--gp-color-neutral-plus-*` in `core/theme.css`) or reference an existing step higher/lower on that scale — every color a designed, explicit value on the scale, not a runtime formula blending two others. `color-mix()` hides the actual resulting color behind an opaque calculation, so nobody can look at the token and know what it renders as, and mixing toward black/white silently desaturates a warm color toward gray instead of staying in the ramp's hue.
