...
- **Per-user stats live in one Scenic view.** `games_played` / `games_won` / `win_percentage` / `play_seconds` / `country` are defined once in `db/views/player_stats_v02.sql` and read through `PlayerStat` — never add a stat method to `User`. To change the definition, run `rails g scenic:view player_stats` for a new version; never edit a migrated `_vNN.sql`. See [docs/plans/player-stats-view.md](docs/plans/player-stats-view.md).
- **Ransack is the filtering/sorting idiom.** Where a model needs user-driven search or sort (the leaderboard is the precedent — see [docs/plans/leaderboard-ransack.md](docs/plans/leaderboard-ransack.md)), reach for `.ransack(params)` rather than hand-rolling a sort whitelist. Every ransackable model must declare `ransackable_attributes` (and `ransackable_associations`, typically `[]`) explicitly — Ransack 4 denies everything by default, and `ransortable_attributes` inherits from `ransackable_attributes` unless overridden separately.
...
- **CSS is served by Propshaft, not bundled.** Styling uses the **Optics** design system (vendored locally in `vendor/optics/`, being phased out — see `docs/plans/own-the-tokens.md`) + **design tokens** + **BEM** class naming, one block per file in `app/assets/stylesheets/components/`. **Token tiers: primitives (`--gp-n-*` / `--gp-w-*` / `--gp-white` / `--gp-black` in `core/theme.css`, non-color in `core/tokens/scale.css`) → roles (`--gp-surface-*` / `--gp-ink-*` in `core/tokens/roles.css`) → `--_gp-*` (private to one component); `--op-*` is now only Optics' own internal tokens**, read by its own not-yet-replaced components (button, sidebar, form, dialog). **Components spend ROLES, never a raw primitive** — if a component needs a tone with no role yet, add the role rather than reaching past it. Color is never taken from Optics — it is all defined in `core/theme.css` (a generated, dark-only OKLCH system), and a bridge block there feeds those values back to the `--op-color-*` tokens Optics reads internally so its own components are painted from our palette. Referencing `--op-color-*` anywhere outside that bridge is a bug (`grep -rn -- '--op-color-' app/assets/stylesheets/ | grep -v core/theme.css` should be empty). Non-color primitives (space/font/radius/border-width/shadow) live in `core/tokens/scale.css` as `--gp-*` too. An older `--gf-*` prefix was retired; if you see it, it's stale. To add styles you just drop a `.css` file there — `stylesheet_link_tag :app` auto-includes it; there is no manifest, `@import`, or CSS build step. The empty `application.css` and webpack CSS build are unused — don't "fix" them. See [docs/frontend.md](docs/frontend.md).
- **Three neutral families, and they are not interchangeable.** `--gp-n-*` is the ramp: the room's tone and its shades (`plus-2`..`minus-10`), anchored on `--gp-n-l-base`. `--gp-white`/`--gp-black` are named absolutes. `--gp-w-1..3` are white pulled back toward the room and are what **ink** is made of. White and black are deliberately *not* ramp steps — they carry no hue, and while they were ramp steps the step size was pinned to whatever made 16 steps land on white. Read `w-2` as "two steps off white," never as a ramp index.
- **Never use `color-mix()` to derive a color.** If the palette doesn't have the shade you need, either tweak the neutral scale's own knobs (`--gp-n-*` in `core/theme.css`) or reference an existing step higher/lower on that scale — every color a designed, explicit value on the scale, not a runtime formula blending two others. In particular, do not mix a color toward `--gp-white`/`--gp-black`: naming those absolutes was not an invitation to blend with them. `color-mix()` hides the actual resulting color behind an opaque calculation, so nobody can look at the token and know what it renders as, and mixing toward black/white silently desaturates a warm color toward gray instead of staying in the ramp's hue.
- **Two frontend docs, two jobs — read the right one.** [DESIGN.md](DESIGN.md) is the visual system: the color model, palette roles, typography, layout grammar, elevation vocabulary, shapes, component specs, and the named rules that govern them. Read it before deciding *what something should look like*. [docs/frontend.md](docs/frontend.md) is the CSS pipeline: how stylesheets load, where files go, BEM mechanics, and the gotchas. Read it before deciding *where the code goes*. Visual guidance does not go in `frontend.md`; pipeline mechanics do not go in `DESIGN.md`.
